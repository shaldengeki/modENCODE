module StatusesHelper
  def display_status_feed_item(status)
    description = link_to(content_tag(:strong, h(status.attempt.reagent.name + (status.success ? " completed " : (status.failure ? " failed " : " is now at ")) + status.step.name)), status) + tag(:br) + h(status.description)
